...
function template_profile_popup()
{
	...
	echo '
		<div class="header">
			<a href="', $scripturl, '?action=profile;u=', $context['user']['id'], ';area=forumprofile;">
...
function template_alerts_popup()
{
	...
	echo '
		<div class="header">
			<a href="', $scripturl, '?action=profile;area=showalerts" class="button">
				', $txt['all_alerts'], '
			</a>
			<div class="options">
				<a href="' . $scripturl . '?action=profile;area=notification;sa=markread;', $context['session_var'], '=', $context['session_id'], '" onclick="return markAlertsRead(this)" title="', $txt['mark_alerts_read'], '">
					<span class="main_icons markread"></span>
				</a>
				<a href="', $scripturl, '?action=profile;area=notification;sa=alerts" title="', $txt['alert_settings'], '">
	...
	else
	{
		...
		{
			echo '
			<', !$details['show_links'] ? 'a href="' . $scripturl . '?action=profile;area=showalerts;alert=' . $id_alert . '" onclick="this.classList.add(\'alert_read\')"' : 'div', ' class="generic_notification">
				<div class="avatar">
...
function template_summary()
{
	...
	<div id="profileview" class="st_profile_2">
		<div id="basicinfo">
			', (!empty($context['member']['id_cover'])? '<div class="profile_cover" style="background-image: url(' . $context['member']['st_profile_cover']['href'] . ')"></div>' : ''), '
			', $context['member']['avatar']['image'], '
	...
	echo '
				<li><a href="', $scripturl, '?action=profile;area=showposts;u=', $context['id_member'], '">', $txt['showPosts'], '</a></li>';

	if ($context['user']['is_owner'] && !empty($modSettings['drafts_post_enabled']))
		echo '
				<li><a href="', $scripturl, '?action=profile;area=showdrafts;u=', $context['id_member'], '">', $txt['drafts_show'], '</a></li>';

	echo '
				<li><a href="', $scripturl, '?action=profile;area=statistics;u=', $context['id_member'], '">', $txt['statPanel'], '</a></li>';
